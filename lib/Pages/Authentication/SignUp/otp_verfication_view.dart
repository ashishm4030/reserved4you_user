...
import 'package:reserve_for_you_user/Pages/Authentication/SignUp/SignUpController.dart';
...
class OtpVerificationView extends StatefulWidget {
  ...
  final String otp;
  OtpVerificationView(
      {this.email, this.last_name, this.device_token, this.device_id, this.device_type, this.join_newsletter, this.first_name, this.password, this.phone, this.otp});
...
class _OtpVerificationViewState extends State<OtpVerificationView> with TickerProviderStateMixin {
  ...
  void initState() {
    ...
    changeBtnText();
    twilioFlutter = TwilioFlutter(accountSid: 'AC40a83e0fadacda88ca315d1e8791abaf', authToken: 'bf31d57794c1e29babdf33ce23a41b68', twilioNumber: '+19802917656');
    ...
    await apiProvider.post(ApiUrl.register, {
      ...
      (value) {
        ...
        if (_responseModel.responseCode == ResponseCodeForAPI.sucessC) {
          ...
          print(user);
          Preferences.preferences.saveBool(key: PrefernceKey.isUserLogin, value: true);
          Preferences.preferences.saveString(key: PrefernceKey.email, value: widget.email);
          Preferences.preferences.saveString(key: PrefernceKey.password, value: widget.password);
          Preferences.preferences.saveString(key: PrefernceKey.loginToken, value: token);
  ...
  void resendSendSms({phoneNUmber}) async {
    print(phoneNUmber);
    var rndnumber = "";
    var rnd = new Random();
    for (var i = 0; i < 6; i++) {
    ...
    try {
      twilioFlutter.sendSMS(toNumber: phoneNUmber, messageBody: 'Dein reserved4you Code ist:$rndnumber');
                      ...
                      child: Column(
                        ...
                        children: [
                          Row(
                            ...
                            children: [
                              Row(
                                children: [
                                  Countdown(
                                    animation: StepTween(begin: levelClock, end: 0).animate(animationController),
                                  ),
                                  Text(
                                    " " + 'seconds'.tr,
                                    style: TextStyle(color: timeUp == true ? Colors.white : Colors.black, fontSize: 28, fontWeight: FontWeight.w600),
                                  ),
                                  SizedBox(
                                    width: 10,
                                  ),
                                  Icon(
                                    ...
                                    color: timeUp == true ? Colors.transparent : Colors.grey,
                                    size: 32,
                                  ),
                                ],
                              ),
                          ...
                          PinPut(
                            ...
                            eachFieldPadding: EdgeInsets.only(top: 18, bottom: 18, left: 20, right: 20),
                          ...
                          CommonBtn(
                            btnText: btnText,
                            isIconAvailable: false,
                            backgroundColor: timeUp == true ? Colors.white70 : Color(AppColor.logoBgColor),
                            textColor: timeUp == true ? Colors.black.withOpacity(0.3) : Colors.black.withOpacity(0.5),
                            padding: EdgeInsets.symmetric(horizontal: 38, vertical: 13),
                            borderRadius: 30.0,
                            fontSize: 15,
                            iconColor: timeUp == true ? Colors.black.withOpacity(0.5) : Colors.black,
                            onButtonPressed: () async {
                              if (timeUp) {
                                return;
                              }
  ...
  resendOTP() async {
    ...
    try {
      ...
      http.Response response = await ApiProvider().post(ApiUrl.resendEmail, body);
      ...
      loader = false;
      setState(() {});
      showTostMessage(message: "Something Went to Wrong Please try again later");
  ...
  Widget CommonBtn({
    ...
    return InkWell(
      ...
      child: Container(
        ...
        decoration:
            BoxDecoration(color: backgroundColor, borderRadius: BorderRadius.circular(borderRadius), border: Border.all(color: backgroundColor ?? backgroundColor), boxShadow: [
          BoxShadow(
          ...
          child: isIconAvailable
              ? Row(
                  ...
                  children: [
                    Text(
                      btnText,
                      style: TextStyle(color: textColor, fontSize: fontSize.toDouble(), fontWeight: FontWeight.bold),
                    ),
                    SizedBox(
                      width: 8,
                    ),
...
class Countdown extends AnimatedWidget {
  ...
  build(BuildContext context) {
    ...
    String timerText = '${clockTimer.inSeconds.remainder(60).toString().padLeft(2, '0')}' == "00" ? "" : '${clockTimer.inSeconds.remainder(60).toString().padLeft(2, '0')}';
